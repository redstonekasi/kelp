...
const SpecialForms = ["def!", "let*", "fn*", "do", "if", "quote", "quasiquote", "unquote", "splice-unquote", "defmacro!"]
...
proc expand(node: KelpNode, env: KelpEnv): KelpNode =
  result = node
  while result.isMacroCall(env):
    ...
    result = fun.body.eval(scope).expand(env)

proc resolve(node: KelpNode, env: var KelpEnv): KelpNode =
  case node.kind
  of kpSymbol:
    env.get(node.str)
  of kpList:
    newList node.list.mapIt(it.eval(env))
  of kpVector:
    newVector node.list.mapIt(it.eval(env))
  of kpTable:
    let res = KelpNode(kind: kpTable)
    for k, v in node.table.pairs:
      res.table[k] = v.eval(env)
    res
  else:
    node
...
proc eval*(node: KelpNode, env: var KelpEnv): KelpNode =
  ...
  while true:
    ...
    if not node.isList: return node.resolve(env)
    ...
    if node.list[0].isSymbol:
      ...
      of "def!":
        ...
        return env.set(key.str, val.eval(env))
      of "defmacro!":
        if node.list.len < 3:
          raise newException(EvalError, "insufficient amount of arguments, expected 2, got " & $(node.list.len - 1))
      ...
      of "qe!":
        return quasiquote(node.list[1])
      of "me!":
        return expand(node.list[1], env)
      else:
        default()
    else:
      default()
